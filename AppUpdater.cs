    ...
    internal class AppUpdater
    {
        ...
        internal bool Check()
        {
            using (var httpClient = new HttpClient())
            {
                ...
                string content = httpClient.GetStringAsync(fileToDownload).Result;
        ...
        private void UpdateFile(string fname)
        {
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    if (File.Exists("new.exe"))
                    {
                        ...
                        string args = $"taskkill /f /im \"{exename}\" && timeout /t 1 && del \"{exepath}\" && ren new.exe \"{exename}\" && \"{exepath}\"";
