...
local function attrToXml(attrTable)
	...
	for k, v in pairs(attrTable) do s = s .. " " .. k .. "=" .. '"' .. v .. '"' end
	return s
end
...
local function getSingleChild(tb)
	...
	if (count == 1) then for k, _ in pairs(tb) do return k end end
	return nil
end
...
local function getFirstValue(tb)
	if type(tb) == "table" then
		for _, v in pairs(tb) do return v end
		return nil
	end
...
function xml2lua.isChildArray(obj)
	for tag, _ in pairs(obj) do if (type(tag) == 'number') then return true end end
	return false
end

function xml2lua.isTableEmpty(obj)
	for k, _ in pairs(obj) do if (k ~= '_attr') then return false end end
	return true
end

function spairs(t)
	local items = {{"StateList", "FilesList"}, {"Name", "Container"}, {"Name", "ExternalEvent"}, {"Name", "ExternalId"}}
	local function custom_sort(a, b)
		-- if b > a then
			for k, v in ipairs(items) do
				if b == v[1] and a == v[2] then return false end
				if a == v[1] and b == v[2] then return true end
			end
		-- end
		return a < b
	end
	...
	if (tagName ~= '_attr') then
		if (type(obj) == 'table') then
			if (xml2lua.isChildArray(obj)) then
				for _, value in pairs(obj) do xml2lua.parseTableToXml(value, tagName, level) end
			elseif xml2lua.isTableEmpty(obj) then
				...
				for tag, value in spairs(obj) do xml2lua.parseTableToXml(value, tag, level + 1) end
